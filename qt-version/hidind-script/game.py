...
game_server = Server()
threading.Thread(target=game_server.serverFunction, daemon=True).start()
...
def init_game():
    if settings.runstop:
        if settings.scripts == 0:
            ...
            try:
                game_server.connection[0].send("MG1;")
            except:
                pass
...
def off_other():
    try:
        game_server.connection[0].send("r1:y1:0;")
        game_server.connection[0].send("r1:y2:0;")
        game_server.connection[0].send("r1:y19:0;")
    except:
        pass
def off_fans():
    try:
        game_server.connection[0].send("r1:y12:0;")
        game_server.connection[0].send("r1:y13:0;")
        game_server.connection[0].send("r1:y14:0;")
        game_server.connection[0].send("r1:y15:0;")
    except:
        pass
def off_strobes():
    try:
        game_server.connection[0].send("r1:y16:0;")
        game_server.connection[0].send("r1:y17:0;")
        game_server.connection[0].send("r1:y18:0;")
    except:
        pass
def off_UV_lamps():
    try:
        game_server.connection[0].send("r1:y3:0;")
        game_server.connection[0].send("r1:y4:0;")
        game_server.connection[0].send("r1:y5:0;")
        game_server.connection[0].send("r1:y6:0;")
        game_server.connection[0].send("r1:y7:0;")
        game_server.connection[0].send("r1:y8:0;")
        game_server.connection[0].send("r1:y9:0;")
        game_server.connection[0].send("r1:y10:0;")
        game_server.connection[0].send("r1:y11:0;")
    except:
        pass
@thread_wraper
def music_play(dt):
        ...
        if settings.runstop:
            if settings.scripts == 0:
                if settings.order_music == 1:
                    ...
                    game_server.connection[0].send("M11;")
                    settings.music_play_event = True
                    music_play(6)
                    ...
                    if tmp == 1:
                        game_server.connection[0].send("M21;")
                    elif tmp == 2:
                        game_server.connection[0].send("M31;")
                    elif tmp == 3:
                        game_server.connection[0].send("M41;")
                    elif tmp == 4:
                        game_server.connection[0].send("M51;")
                ...
                if settings.order_music == 1:
                    settings.order_music+=1
                    game_server.connection[0].send("M71;")
                    settings.music_play_event = True
                    music_play(11)
                    ...
                    tmp = random.randint(1,4)
                    if tmp == 1:
                        game_server.connection[0].send("M81;")
                    elif tmp == 2:
                        game_server.connection[0].send("M91;")
                    elif tmp == 3:
                        game_server.connection[0].send("MA1;")
                    elif tmp == 4:
                        game_server.connection[0].send("MB1;")
...
def strobe_music_play(dt):
    try:
        ...
        if settings.bonuses['strobes']:
            ...
            print(tmp)
            if tmp == 1:
                game_server.connection[0].send("MD1;")
            elif tmp == 2:
                game_server.connection[0].send("ME1;")
            elif tmp == 3:
                game_server.connection[0].send("MF1;")
    except:
        pass
def play_end_music():
    try:
        if settings.scripts == 0:
            game_server.connection[0].send("M61;")

        if settings.scripts == 1:
            game_server.connection[0].send("MC1;")
    except:
        pass
...
def stop_events():
    ...
    if settings.outs['RunStopLamp']:
        action_runstop_lamp(0)
    try:
        game_server.connection[0].send("M00;")
    except:
        pass
    if settings.runstop:
        play_end_music()

def playMusic(self, rpi: str, track: str):
    game_server.send_message(f'{rpi}:play:{track};')

def pauseMusic(self, rpi: str, track: str):
    game_server.send_message(f'{rpi}:pause:{track};')

def stopMusic(self, rpi: str, track: str):
    game_server.send_message(f'{rpi}:stop:{track};')

def changeVolume(self, rpi: str, volume: int):
